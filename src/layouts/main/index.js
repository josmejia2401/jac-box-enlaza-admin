      ...
      <>
        <div className="min-h-full min-h-screen bg-background">
          <Disclosure as="nav" className="bg-primary shadow">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <div className="flex h-16 items-center justify-between">
                      ...
                      {navigation.map((item) => (
                        item.children && item.children.length > 0 ? (
                          <Menu key={item.name} as="div" className="relative inline-block text-left">
                            ...
                            <MenuItems
                              transition
                              className="absolute left-0 z-50 mt-2 w-48 origin-top-left rounded-md bg-surface shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                              <div className="py-1">
                                  ...
                                  <MenuItem key={child.name}>
                                    {({ active }) => (
                                      <Link
                                        to={child.href}
                                        className={classNames(
                                          active ? 'bg-secondary text-primary' : 'text-text',
                ...
                <div className="hidden md:block">
                  <div className="ml-4 flex items-center md:ml-6">
                    ...
                    <Menu as="div" className="relative ml-3">
                      <div>
                        <MenuButton className="relative flex max-w-xs items-center rounded-full bg-secondary text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-secondary focus:outline-none transition">
                      ...
                      <MenuItems
                        transition
                        className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-surface py-1 shadow-lg ring-1 ring-black/5 transition"
                      >
                        {userNavigation.map((item) => (
                          <MenuItem key={item.name}>
                            <Link
                              to={item.href}
                              className="block px-4 py-2 text-sm text-text hover:bg-secondary hover:text-primary transition"
                              onClick={async (e) => {
                                e.preventDefault();
                                this.setState({
                ...
                <div className="-mr-2 flex md:hidden">
                  {/* Mobile menu button */}
                  <DisclosureButton className="group relative inline-flex items-center justify-center rounded-md bg-secondary p-2 text-primary hover:bg-secondary hover:text-primary focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-secondary focus:outline-none transition">
            ...
            {/* Mobile menu panel */}
            <DisclosurePanel className="md:hidden bg-surface">
                    ...
                    <div key={item.name} className="">
                      <button
                        ...
                        className={classNames(
                          ...
                          item.current ? 'bg-secondary text-primary' : 'text-text hover:bg-secondary hover:text-primary'
                        )}
                      ...
                      >
                        ...
                        {mobileSubmenusOpen[idx] ? (
                          <ChevronUpIcon className="h-5 w-5 text-text" aria-hidden="true" />
                        ) : (
                          <ChevronDownIcon className="h-5 w-5 text-text" aria-hidden="true" />
                        )}
                      </button>
                      <div
                      ...
                      >
                        {item.children.map((child) => (
                          <a
                            key={child.name}
                            href={child.href}
                            className="block rounded-md px-3 py-2 text-base font-medium text-text hover:bg-secondary hover:text-primary transition"
                          >
                            {child.name}
                          </a>
                        ))}
              ...
              <div className="border-t border-muted pt-4 pb-3">
                <div className="flex items-center px-5">
                  ...
                  <div className="ml-3">
                    <div className="text-base/5 font-medium text-text">{user.name || "jose"}</div>
                    <div className="text-sm font-medium text-muted">{user.email || "email"}</div>
                  </div>
                </div>
